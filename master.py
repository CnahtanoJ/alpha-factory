    ...
    if best_tf:
            ...
            json.dump(
                {
                    ...
                    "sharpe": (
                        float(best_sharpe) if best_sharpe != -float("inf") else 0.0
                    ),
                    "spearman": (
                        float(best_spearman) if best_spearman != -float("inf") else 0.0
                    ),
        ...
        try:
            ...
            with open(config_path, "rb") as data:
                s3.put_object(
                    Bucket=AWS_BUCKET, Key="live_config.json", Body=data.read()
                )
            print(f"   live_config.json uploaded to S3 bucket '{AWS_BUCKET}'.")
        except Exception as e:
            print(f"   live_config.json S3 upload failed: {e}")
...
def _run_report_logic(args):
    ...
    timeframes = sorted(
        [t.strip() for t in timeframes], key=lambda x: tf_order.get(x, 99)
    )
    all_results = {}
    ...
    for tf in timeframes:
        ...
        if isinstance(cycle_results, dict) and "status" not in cycle_results:
            ...
            print(f"Skipping {tf} due to training failure or no data.")
...
def cmd_status(args):
    ...
    cursor.execute(
        "SELECT symbol, timeframe, COUNT(*) as count, MIN(timestamp), MAX(timestamp) FROM ohlcv GROUP BY symbol, timeframe"
    )
    ...
    if not rows:
        ...
        for row in rows:
            ...
            print(
                f"{symbol:<15} | {tf:<5} | {count:<8,} | {start_dt:<20} | {end_dt:<20}"
            )
...
def cmd_health(args):
    ...
    for tf in timeframes:
        ...
        if os.environ.get("TESTNET_MODE", "False").lower() == "true":
            key = os.environ.get(
                f"TESTNET_PRIVATE_KEY_{suffix}", os.environ.get("TESTNET_PRIVATE_KEY")
            )
            addr = os.environ.get(
                f"TESTNET_ACCOUNT_ADDRESS_{suffix}",
                os.environ.get("TESTNET_ACCOUNT_ADDRESS"),
            )
            mode = "TESTNET"
        else:
            key = os.environ.get(
                f"MAINNET_PRIVATE_KEY_{suffix}", os.environ.get("MAINNET_PRIVATE_KEY")
            )
            addr = os.environ.get(
                f"MAINNET_ACCOUNT_ADDRESS_{suffix}",
    ...
    if os.environ.get("TELEGRAM_BOT_TOKEN") and os.environ.get("TELEGRAM_CHAT_ID"):
        ...
        if args.ping_telegram:
            ...
            send_telegram_message(
                "*Alpha Factory Health Check*: Connectivity Verified."
            )
    else:
...
def cmd_audit(args):
    ...
    if not partitions:
        print("  No matching data partitions found to audit.")
        return

    print(f"  Auditing {len(partitions)} partitions...\n")
    print(
        f"  {'Symbol':<15} {'TF':<6} {'Grade':<6} {'Gaps':>5} {'Spikes':>6} {'Health':>7}"
    )
    ...
    for p in partitions:
        ...
        grade_color = (
            "" if res["health_score"] >= 90 else "" if res["health_score"] >= 70 else ""
        )

        print(
            f"  {res['symbol']:<15} {res['timeframe']:<6} {grade_color} {res['grade']:<3} {res['gaps_found']:>5} {res['anomalies']:>6} {res['health_score']:>6.1f}%"
        )
...
Recommended first-time workflow:
  ...
  3. python master.py report
    ...
    p_ingest = subparsers.add_parser(
        "ingest", help="Download historical data to local DB"
    )
    ...
    p_ingest.add_argument(
        "--years", type=int, default=3, help="Target years of history (default: 3)"
    )
    p_ingest.add_argument(
        "--start-year",
        type=int,
    ...
    p_report = subparsers.add_parser(
        "report", help="Run intelligence cycle + generate report"
    )
    ...
    p_full = subparsers.add_parser(
        "full", help="Run complete end-to-end cycle (Ingest? → Sync → Train → Report)"
    )
    # Include Ingest Args
    p_full.add_argument(
        "--symbols",
    ...
    p_full.add_argument(
        "--top", type=int, default=0, help="Auto-discover top N symbols to bootstrap"
    )
    p_full.add_argument(
        "--years", type=int, default=3, help="Years of history to fetch (default: 3)"
    )
    p_full.add_argument(
        "--start-year",
        type=int,
    ...
    p_audit = subparsers.add_parser(
        "audit", help="Check data integrity (gaps and spikes)"
    )
    p_audit.add_argument(
        "--market", choices=["spot", "futures"], help="Market to audit"
    )
    p_audit.add_argument(
        "--symbols", help="Specific symbols to audit (comma-separated)"
    )
    ...
    p_health = subparsers.add_parser(
        "health", help="Verify all sub-account credentials and connectivity"
    )
