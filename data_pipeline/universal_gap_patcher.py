...
class UniversalGapPatcher:
    ...
    def patch_ohlcv(self, dry_run=True):
        ...
        partitions = self.conn.execute("SELECT DISTINCT symbol, timeframe, market FROM ohlcv").fetchall()
    ...
    def patch_index_ohlcv(self, dry_run=True):
        ...
        partitions = self.conn.execute("SELECT DISTINCT symbol, timeframe FROM index_ohlcv").fetchall()
        ...
        for row in partitions:
            ...
            for gap in gaps:
                ...
                if not dry_run:
                    inserted = self._patch_daily_zip(
                        table='index_ohlcv', symbol=symbol, ts=ts, next_ts=next_ts,
                        time_col='timestamp', data_type='indexPriceKlines', timeframe=timeframe,
                        extra_cols={},
                        insert_cols=['timestamp', 'symbol', 'timeframe', 'open', 'high', 'low', 'close']
                    )
                    if inserted == 0:
                        print("    [INFO] Marking gap as unfillable to prevent future re-checks.")
                        self.conn.execute("INSERT OR IGNORE INTO unfillable_gaps (table_name, symbol, timeframe, start_ts, end_ts) VALUES (?, ?, ?, ?, ?)",
    ...
    def patch_symbol_metrics(self, dry_run=True):
        ...
        partitions = self.conn.execute("SELECT DISTINCT symbol FROM symbol_metrics").fetchall()
        ...
        for row in partitions:
            ...
            for gap in gaps:
                ...
                if not dry_run:
                    inserted = self._patch_daily_zip(
                        table='symbol_metrics', symbol=symbol, ts=ts, next_ts=next_ts,
                        time_col='create_time', data_type='metrics', timeframe='5m', 
                        extra_cols={},
                        insert_cols=['timestamp', 'create_time', 'symbol', 'sum_open_interest', 'sum_open_interest_value', 
    ...
    def patch_funding_rate(self, dry_run=True):
        ...
        partitions = self.conn.execute("SELECT DISTINCT symbol FROM funding_rate").fetchall()
    ...
    def _patch_daily_zip(self, table, symbol, ts, next_ts, time_col, data_type, timeframe, extra_cols, insert_cols):
        ...
        while current_date <= end_date:
            ...
            # Fetch daily data
            df = self.vision.fetch_data(symbol, timeframe, current_date.year, current_date.month, daily_date=date_str, data_type=data_type)
            ...
            if not df.empty:
                ...
                if not df_to_insert.empty:
                    ...
                    if timeframe: df_to_insert['timeframe'] = timeframe
                    for k, v in extra_cols.items():
                        df_to_insert[k] = v
                    # Ensure timestamp exists if expected
                    if 'timestamp' in insert_cols and 'timestamp' not in df_to_insert.columns and time_col in df_to_insert.columns:
                        df_to_insert['timestamp'] = df_to_insert[time_col]
                        
                    # Filter columns exactly to the schema
                    available_cols = [c for c in insert_cols if c in df_to_insert.columns]
                    ...
                    print(f"        [INFO] ZIP contained no rows within the gap boundaries.")
            else:
    ...
    def _patch_monthly_zip(self, table, symbol, ts, next_ts, time_col, data_type, timeframe, extra_cols, insert_cols):
        ...
        while current_date <= end_date:
            print(f"      [FETCH] Fetching {data_type} Monthly ZIP for {current_date.strftime('%Y-%m')}...")
            
            df = self.vision.fetch_data(symbol, "1d", current_date.year, current_date.month, data_type=data_type)
