...
class DataAuditor:
    ...
    def audit_structural_integrity(self):
        ...
        for table in tables["name"]:
            ...
            if table == "ohlcv":
                ...
                dist = pd.read_sql_query(
                    "SELECT timeframe, COUNT(*) as count FROM ohlcv GROUP BY timeframe",
                    conn,
                )
        ...
        if sync_df.empty:
            ...
            print("  Warning: No 'klines' (Binance Vision) sync state found.")
        else:
    ...
    def audit_join_integrity(self, symbol="BTC/USDT", timeframe="1h"):
        ...
        LIMIT 5
        """
        index_joined = pd.read_sql_query(
            index_query, conn, params=(symbol, timeframe, symbol)
        )
    ...
    def audit_pair(self, symbol, timeframe, market, anomaly_threshold=0.20):
        ...
        df["pct_move"] = (df["close"] - df["open"]).abs() / df["open"].replace(
            0, np.nan
        )
        ...
        gap_ratio = (
            results["missing_candles"]
            / (results["total_rows"] + results["missing_candles"])
...
if __name__ == "__main__":
    ...
    for p in parts[:5]:
        ...
        if res["status"] == "OK":
            ...
            print(
                f"  Rows   : {res['total_rows']:,} | Gaps: {res['gaps_found']} instances ({res['missing_candles']} candles)"
            )
