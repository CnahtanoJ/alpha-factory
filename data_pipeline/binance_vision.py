...
class BinanceVision:
    ...
    def _get_url(
        self, symbol, timeframe, year, month=None, daily_date=None, data_type="klines"
    ):
        ...
        if (
            clean_symbol.startswith("k")
            and len(clean_symbol) > 1
            and clean_symbol[1].isupper()
        ):
    ...
    def fetch_data(
        ...
        try:
            ...
            if (
                not first_val.replace(".", "", 1).isdigit()
                and not first_val.replace("-", "")
                .replace(" ", "")
                .replace(":", "")
                .isdigit()
            ):
    ...
    def fetch_history_range(
        ...
        for year in range(start_year, now.year + 1):
            ...
            for month in range(m_start, 13):
                ...
                # Try monthly first
                df = self.fetch_data(
                    symbol, timeframe, year, month, data_type=data_type
                )
                ...
                if not df.empty:
                    ...
                    if year >= now.year - 1 and data_type != "fundingRate":
                        print(
                            f"  Monthly missing for {year}-{month:02d}. Trying Daily..."
                        )
                        d_start = (
                            start_day
                        ...
                        for day in range(d_start, 32):
                            ...
                            df_daily = self.fetch_data(
                                symbol,
                                timeframe,
                                year,
                                month,
                                daily_date=daily_str,
                                data_type=data_type,
                            )
        ...
        if "timestamp" not in all_dfs[0].columns:
            sort_col = (
                "calc_time" if "calc_time" in all_dfs[0].columns else "create_time"
            )

        return (
            pd.concat(all_dfs).drop_duplicates(subset=[sort_col]).sort_values(sort_col)
        )
