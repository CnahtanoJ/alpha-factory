...
class SyncManager:
    ...
    def sync_from_binance_vision(
        ...
        """
        print(
            f"Downloading {symbol} ({timeframe}) [{market}] {data_type} from Binance Vision..."
        )
        ...
        if latest:
            try:
                ...
                print(
                    f"  Resuming from {effective_start_year}-{effective_start_month:02d}-{effective_start_day:02d}"
                )
            except (OSError, ValueError):
                print(
                    f"  Corrupted sync state detected for {symbol}. Starting from scratch."
                )
                effective_start_year = start_year
                effective_start_month = 1
                effective_start_day = 1
        ...
        if (
            binance_symbol.startswith("k")
            and len(binance_symbol) > 1
            and binance_symbol[1].isupper()
        ):
        ...
        if df.empty:
            print(f"  No data found on Binance Vision for {symbol} ({data_type}).")
            ...
            self.update_sync_state(
                symbol, timeframe, market, data_type, final_earliest, now_ms
            )
            return 0
            ...
            if valid_min_ms <= ts <= valid_max_ms:
                if data_type == "klines":
                    ...
                    insert_data.append(
                        (
                            ts,
                            int(row.get("create_time", ts)),
                            symbol,
                            float(row.get("sum_open_interest", 0)),
                            float(row.get("sum_open_interest_value", 0)),
                            float(row.get("count_toptrader_long_short_ratio", 0)),
                            float(row.get("sum_toptrader_long_short_ratio", 0)),
                            float(row.get("count_long_short_ratio", 0)),
                            float(row.get("sum_long_short_ratio", 0)),
                            float(row.get("count_taker_long_short_vol_ratio", 0)),
                            float(row.get("sum_taker_long_short_vol_ratio", 0)),
                        )
                    )
        ...
        if not insert_data:
            print(f"  No valid timestamps found in {symbol} Binance Vision data.")
            return 0
        ...
        cur_earliest, cur_latest = self.get_sync_state(
            symbol, timeframe, market, data_type
        )
        final_earliest = (
            min(new_earliest, cur_earliest) if cur_earliest else new_earliest
        )
        final_latest = max(new_latest, cur_latest) if cur_latest else new_latest

        self.update_sync_state(
            symbol, timeframe, market, data_type, final_earliest, final_latest
        )
        self.conn.commit()
    ...
    def sync_symbol(
        ...
        **kwargs,
        ...
        self.sync_from_binance_vision(
            symbol, timeframe, market, start_year, data_type="klines"
        )
        self.sync_from_binance_vision(
            symbol, timeframe, market, start_year, data_type="indexPriceKlines"
        )
        self.sync_from_binance_vision(
            symbol, timeframe, market, start_year, data_type="metrics"
        )
        self.sync_from_binance_vision(
            symbol, timeframe, market, start_year, data_type="fundingRate"
        )
