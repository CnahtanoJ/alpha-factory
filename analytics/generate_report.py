...
def build_report_section(timeframe: str, cycle_results: dict) -> str:
    ...
    if sim and sim.get("n_rebalances", 0) > 0:
        ...
        if "mc_stats" in sim:
            mc = sim["mc_stats"]
            md += "#### Robustness Analysis (Monte Carlo)\n"
            md += f"- **Probability of Profit**: {mc['prob_profit']:.1%}\n"
            md += f"- **95% CI**: [{mc['ci_lower']:+.2%}, {mc['ci_upper']:+.2%}]\n\n"
    else:
        md += "> No OOS simulation available. This is the first training run.\n\n"
...
def generate_report(multi_results: dict) -> str:
    ...
    for tf, res in multi_results.items():
        ...
        sha = (
            f"{sim['sharpe']:.2f} ({sim.get('annualized_sharpe', 0.0):.2f} Ann.)"
            if sim
            else "N/A"
        )
    ...
    best_tf = max(
        multi_results.keys(),
        key=lambda k: multi_results[k]
        .get("model_meta", {})
        .get("validation_spearman", 0),
    )
...
if __name__ == "__main__":
    print("Use 'python master.py report' to generate a report with full cycle data.")
