...
def executor_handler(event, context):
    ...
    # Download the latest config to see which timeframe is "King"
    try:
    ...
    # Resolve window size (12 bars)
    window_map = {'15m': 180, '1h': 720, '4h': 2880} # Minutes
    rebalance_window_mins = window_map.get(timeframe, 180)
    
    now_ts = int(time.time())
    mins_since_rebalance = (now_ts - last_rebalance) / 60
    
    # Forced task override from event (for manual testing)
    task = event.get("task") 
    if not task:
    ...
    # --- TASK B: TP/SL MANAGEMENT ONLY (High Frequency) ---
    if task == "manage_tpsl":
    ...
    # --- TASK C: FULL REBALANCE (Low Frequency) ---
    try:
        ...
        target_longs = (kept_longs + eligible_longs[~eligible_longs['symbol'].isin(current_longs)]['symbol'].tolist())[:BASKET_N]
        ...
        target_shorts = (kept_shorts + eligible_shorts[~eligible_shorts['symbol'].isin(current_shorts)]['symbol'].tolist())[-BASKET_N:]
        ...
        if not target_rows.empty:
            ...
            target_rows['rp_weight'] = target_rows['inv_vol'] / target_rows['inv_vol'].sum()
            ...
            for target_coin in target_basket:
                ...
                if target_coin not in portfolio:
                    ...
                    if risk.check_execution_safety(target_coin, is_buy, current_price, poc_price):
                        risk.execute_logic(target_coin, signal, timeframe, atr_pct, portfolio, user_state, open_orders, override_usd=symbol_to_usd.get(target_coin))
                else:
            ...
            logger.info(f"🏁 Rebalance task complete.")
            # 3. THE SHIELD
