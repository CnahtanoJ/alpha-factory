...
class AssetManager:
    ...
    def refresh_universe(self):
        ...
        for attempt in range(3):
            try:
                ...
                self.universe = {str(a['name']).upper(): a for a in raw_meta['universe']}
                return # Success!
            except Exception as e:
                logger.warning(f"⚠️ Meta fetch failed (Attempt {attempt+1}/3): {e}")
                time.sleep(0.5)
    ...
    def get_safe_tp_size(self, coin, total_pos_size, tp_pct):
        ...
        if usd_value < MIN_NOTIONAL:
            print(f"⚠️ TP Adjustment: Chunk ${usd_value:.2f} too small. Selling ALL.")
        ...
        if 0 < remaining_val < MIN_NOTIONAL:
            print(f"⚠️ TP Adjustment: Leftover ${remaining_val:.2f} is dust. Selling ALL.")
            return total_pos_size # Sell Everything
    ...
    def get_clean_candles(self, coin, interval="15m", limit=5000, use_db=False):
        ...
        if use_db:
            try:
                ...
                if df.empty:
                    logger.warning(f"MarketData: No local data for {db_symbol} [{interval}].")
                    return pd.DataFrame()
        ...
        try:
            ...
            raw = self.info.candles_snapshot(coin, interval, start_time_ms, current_time_ms)
