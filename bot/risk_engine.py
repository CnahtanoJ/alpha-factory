...
from bot.config import MARGIN_LIMIT, DCA_LIMIT, POC_GRAVITY_FILTER_ACTIVE
...
class RiskEngine:
    ...
    def get_unified_equity(self):
        ...
        try:
            ...
            summary = user_state.get("crossMarginSummary") or user_state.get(
                "marginSummary", {}
            )
    ...
    def check_safety(self):
        ...
        try:
            ...
            if val <= 0:
                logger.error(
                    "CRITICAL: Unified Account Value is 0 or negative! Stopping."
                )
                return False
            ...
            if curr_usage > MARGIN_LIMIT:
                logger.warning(
                    f"HIGH RISK: Unified Margin Usage {curr_usage:.2%}. Limit: {MARGIN_LIMIT:.2%}"
                )
                send_telegram_message(
                    f"High Margin Alert: {curr_usage:.2%}. Pausing new entries."
                )
                return False
    ...
    def clean_global_zombies(self, portfolio, open_orders):
        ...
        if not self.info or not self.exchange:
            logger.error(
                "Cannot clean zombies: RiskEngine missing 'info' or 'exchange'."
            )
            return
        ...
        for coin, orders in orders_by_coin.items():
            ...
            if is_pending_entry:
                logger.info(
                    f"{coin}: Ignoring Global Clean (Active Limit Entry detected)"
                )
                continue

            # C. KILL ZOMBIES
            logger.warning(
                f"GLOBAL ZOMBIE DETECTED: {coin} has orders but only dust/no position."
            )
            send_telegram_message(
                f"GLOBAL CLEAN: Canceling all {coin} orders (Zombie/Dust)."
            )
    ...
    def get_live_sensor_data(self, target_coin):
        # GUARD CLAUSE: Ensure the tool exists
        if not self.info:
        ...
        try:
            ...
            logger.error(f"Sensor error for {target_coin}: {e}")
            return None
    ...
    def check_execution_safety(self, coin, is_buy, current_price, poc_price):
        ...
        if not sensor:
            logger.error(f"Sensor failure on {coin}. Aborting execution for safety.")
            return False
        ...
        if oi_usd < MIN_OI:
            logger.warning(
                f"REJECTED: {coin} OI is only ${oi_usd:,.0f} (Needs ${MIN_OI:,.0f}). Danger of slippage."
            )
            return False
        ...
        if is_buy and funding > MAX_FUNDING:
            logger.warning(
                f"REJECTED: {coin} is overcrowded. Funding rate too high ({funding:.4f})."
            )
            return False

        if not is_buy and funding < -MAX_FUNDING:
            logger.warning(
                f"REJECTED: {coin} short is overcrowded. Funding rate too negative ({funding:.4f})."
            )
            return False

        # 4. RULE 3: POC Gravity
        if POC_GRAVITY_FILTER_ACTIVE:
            if is_buy and current_price > poc_price:
                logger.warning(
                    f"REJECTED: {coin} Long at ${current_price:.4f} is above the POC (${poc_price:.4f})."
                )
                return False

            if not is_buy and current_price < poc_price:
                logger.warning(
                    f"REJECTED: {coin} Short at ${current_price:.4f} is below the POC (${poc_price:.4f})."
                )
                return False
        else:
            if is_buy and current_price > poc_price:
                logger.info(
                    f"POC WARNING: {coin} Long at ${current_price:.4f} is above the POC (${poc_price:.4f}). (Filter inactive)"
                )

            if not is_buy and current_price < poc_price:
                logger.info(
                    f"POC WARNING: {coin} Short at ${current_price:.4f} is below the POC (${poc_price:.4f}). (Filter inactive)"
                )

        # If it passes the bouncer, greenlight the trade
        logger.info(
            f"CLEARED: {coin} execution safe. OI: ${oi_usd:,.0f} | Funding: {funding:.4f}"
        )
        return True
    ...
    def parse_portfolio(self, user_state, all_mids, dust_threshold=1.0):
        ...
        for p in user_state["assetPositions"]:
            ...
            if abs(sz * price) > dust_threshold:
                portfolio[coin] = (
                    pos  # Save the entire clean dictionary under the coin's name
                )
    ...
    def cancel_all_orders(self, coin):
        ...
        logger.info(f"Found {len(coin_orders)} orders to cancel for {coin}...")
    ...
    def execute_logic(
        ...
        if pos_details:
            ...
            original_base_unit = (
                entry_usd / avg_entry_px if avg_entry_px > 0 else base_sz_unit
            )
            current_count = (
                int(round(abs(pos_size) / original_base_unit))
                if original_base_unit > 0
                else 1
            )
        else:
            pos_size = 0.0
            ...
            consecutive_losses = self.memory.get(
                "GLOBAL", "consecutive_losses", default=0
            )
            if consecutive_losses >= 3:
                logger.warning(
                    f"CIRCUIT BREAKER TRIPPED: {consecutive_losses} consecutive losses. Pausing {coin} entry."
                )
                return
        ...
        if signal == "EXIT" and has_position:
            ...
            send_telegram_message(
                f"STRATEGY EXIT: {coin} | Closing position at Middle Band."
            )
            ...
            close_px = (
                curr_mid * (1 + 0.015) if is_buy_close else curr_mid * (1 - 0.015)
            )
        ...
        if has_position:
            ...
            HOLD_MAP = {"5m": 900, "15m": 2700, "1h": 10800}
            ...
            if signal != curr_side:
                ...
                if cond_time or cond_profit or cond_emergency:
                    ...
                    logger.info(
                        f"FLIP SIGNAL: {curr_side} -> {signal} after {time_held/60} minutes. Switching sides!"
                    )
                    ...
                    logger.info(
                        f"IGNORING {signal}: Holding {curr_side} (TP1 not hit yet)."
                    )
                    return
            ...
            if not just_flipped:
                ...
                dist_threshold = (
                    avg_entry_px * (1 - self.dca_spacing)
                    if is_long
                    else avg_entry_px * (1 + self.dca_spacing)
                )
                should_dca = (
                    (mid_px < dist_threshold) if is_long else (mid_px > dist_threshold)
                )

                if not should_dca:
                    current_pullback = (
                        (avg_entry_px - mid_px) / avg_entry_px
                        if is_long
                        else (mid_px - avg_entry_px) / avg_entry_px
                    )
                ...
                send_telegram_message(
                    f"{coin}: DCA Triggered at {mid_px}! Price gap sufficient."
                )
                ...
                pending_dca = next(
                    (
                        ...
                        for o in open_orders
                        if o["coin"] == coin
                        and o["isTrigger"] == False  # Limit/Market orders
                ...
                if pending_dca:
                    logger.warning(
                        f"DCA Aborted: Found pending order {pending_dca['oid']}. Waiting for fill."
                    )
                    return

                logger.info(
                    f"PRE-DCA CLEANUP: Cancelling all orders for {coin} to reset TP/SL."
                )
                try:
                    self.cancel_all_orders(coin)
                    time.sleep(1)
        ...
        try:
            res = self.exchange.order(
                coin, is_buy, sz, final_limit_px, {"limit": {"tif": "Ioc"}}
            )
            logger.info(f"API RESPONSE: {res}")

            if res["status"] == "ok":
                filled_val = (
                    res["response"]["data"]["statuses"][0]
                    .get("filled", {})
                    .get("totalSz", 0)
                )
                ...
                if filled_float > 0:
                    if filled_float < sz:
                        send_telegram_message(
                            f"PARTIAL FILL: {coin} intended {sz}, got {filled_float}!"
                        )
                        logger.warning(
                            f"PARTIAL FILL: {coin} intended {sz}, got {filled_float}!"
                        )
                    ...
                    logger.warning(f"Entry Failed (0 filled).")
                    send_telegram_message(f"Entry Failed (0 filled).")
    ...
    def sync_unified_orders(self, coin, current_atr_pct, portfolio):
        ...
        if self.assets.round_size(coin, total_sz) <= 0:
            logger.warning(
                f"{coin} position size {total_sz} is dust. Skipping Unified Orders."
            )
            return
        ...
        logger.info(
            f"PLACING INITIAL SL: Total {total_sz} @ {avg_entry} | ATR: {current_atr_pct:.2%}"
        )

        sl_mult = 1.0
        hard_sl_pct = current_atr_pct * sl_mult
        d = 1 if is_buy_pos else -1
        sl_px = self.assets.get_price_precision(
            coin, avg_entry * (1 - (hard_sl_pct * d))
        )
    ...
    def sync_trailing_stop(self, coin, current_atr_pct, portfolio, open_orders):
        ...
        if coin not in portfolio:
            if self.memory.get(coin, "entry_time", default=0) > 0:
                ...
                if is_profitable:
                    ...
                    streak = 0
                    logger.info(
                        f"{coin} trailing stop hit in profit/BE ({entry_px} -> {sl_px}). Streak reset."
                    )
                else:
                    streak += 1
                    self.memory.set("GLOBAL", "consecutive_losses", streak)
                    logger.info(
                        f"{coin} trailing stop hit at a loss ({entry_px} -> {sl_px}). Consecutive losses: {streak}"
                    )

                logger.info(f"DETECTED EXIT: {coin} position is gone.")
                send_telegram_message(
                    f"NOTIFICATION: {coin} Position Closed. Current Streak Metric: {streak}"
                )
                self.memory.clear(coin)
            return
        ...
        if (
            stored_entry != 0
            and abs(current_entry - stored_entry) / current_entry > 0.005
        ):  # 0.5% diff
            logger.info(
                f"NEW TRADE DETECTED! Entry changed {stored_entry} -> {current_entry}"
            )
        ...
        # 1. SOFT STOP WATCHDOG
        soft_dist = 0.01
        soft_limit = (
            entry_px * (1 - soft_dist) if is_long else entry_px * (1 + soft_dist)
        )
        soft_hit = (curr_px < soft_limit) if is_long else (curr_px > soft_limit)

        if soft_hit:
            if not self.memory.get(coin, "soft_warned"):
                logger.warning(f"SOFT STOP: {coin} breached 1%!")
                send_telegram_message(
                    f"SOFT STOP ALERT: {coin} is down > 1% @ {curr_px}."
                )
        ...
        sl_improved = (
            (new_sl_px > current_sl_px * 1.002)
            if is_long
            else (new_sl_px < current_sl_px * 0.998)
        )

        if sl_improved:
            safe_new_sl = self.assets.get_price_precision(coin, new_sl_px)
            logger.info(
                f"TRAILING STOP TRIGGERED: Moving {coin} SL from {current_sl_px} to {safe_new_sl}"
            )
            ...
            try:
                ...
                if res["status"] == "ok":
                    ...
                    send_telegram_message(
                        f"TRAILING STOP: {coin} SL raised to {safe_new_sl} (Current Price: {curr_px})"
                    )
    ...
    def close_active_position(
        self, active_coin, all_mids, temp_assets, portfolio, aws_bucket
    ):
        """
        ...
        is_buy = (
            pos_size < 0
        )  # If Short (-), we Buy to close. If Long (+), we Sell to close.
        slippage = 0.015
        ...
        try:
            ...
            if res["status"] != "ok":
                ...
                send_telegram_message(
                    f"SWITCH ABORTED: Could not close {active_coin}. Error: {error_msg}"
                )
                return False
            ...
            if filled_val == 0:
                ...
                send_telegram_message(
                    f"SWITCH ABORTED: 0 filled on close for {active_coin}."
                )
                return False
            elif filled_val < abs(pos_size):
                logger.warning(
                    f"PARTIAL CLOSE: {active_coin} intended {abs(pos_size)}, got {filled_val}."
                )
                send_telegram_message(
                    f"PARTIAL CLOSE: {active_coin} intended {abs(pos_size)}, got {filled_val}."
                )
            ...
            if pnl_pct < 0:
                ...
                self.memory.set("GLOBAL", "consecutive_losses", streak)
                logger.info(
                    f"{active_coin} closed at a loss ({pnl_pct:.2%}). Consecutive losses: {streak}"
                )
            else:
                self.memory.set("GLOBAL", "consecutive_losses", 0)
                logger.info(
                    f"{active_coin} closed in profit/BE ({pnl_pct:.2%}). Streak reset."
                )
