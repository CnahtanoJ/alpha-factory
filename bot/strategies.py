...
class SimpleBreakout(VectorStrategy):
    ...
    def get_signal_column(self, df):
        ...
        if self.use_vwap:
            ...
            vwap = (typical_price * df["volume"]).groupby(df.index.date).cumsum() / df[
                "volume"
...
class MACDStrategy(VectorStrategy):
    ...
    def get_signal_column(self, df):
        ...
        buy_cond = (
            (macd > signal_line) & (macd.shift(1) <= signal_line.shift(1)) & is_trending
        )
        sell_cond = (
            (macd < signal_line) & (macd.shift(1) >= signal_line.shift(1)) & is_trending
        )
...
class BollingerSqueezeBreakout(VectorStrategy):
    ...
    def __init__(
        self, bb_window=20, bb_std=2, squeeze_lookback=50, use_vwap=False, use_htf=False
    ):
...
class RSIStrategyFast(VectorStrategy):
    """
...
class BollingerReversion(VectorStrategy):
    ...
    def get_signal_column(self, df):
        ...
        buy_cond = (
            (df["close"] < lower) & (bbw > self.min_bbw) & (df["close"] >= prev_close)
        )

        # Short: Live price is above the upper band, volatility is high, AND the live candle is red
        sell_cond = (
            (df["close"] > upper) & (bbw > self.min_bbw) & (df["close"] <= prev_close)
        )
...
class OrderFlowStrategy(VectorStrategy):
    ...
    def get_signal_column(self, df):
        ...
        typical_price = (df["high"] + df["low"] + df["close"]) / 3
        poc = (typical_price * df["volume"]).rolling(self.lookback).sum() / df[
            "volume"
        ].rolling(self.lookback).sum()
...
class OrderFlowSweep(VectorStrategy):
    ...
    def get_signal_column(self, df):
        ...
        cvd_slope = cvd.diff(
            self.cvd_lookback
        )  # Short lookback to catch immediate divergence
...
STRATEGY_CONFIG = {
    ...
    "MACDStrategy": {
        ...
        "params": {
            ...
            "use_htf": [
                True
            ],  # Trend following REQUIRES higher timeframe confirmation.
        },
    },
    "BBSqueezeBreakout": {
        ...
        "params": {
            ...
            "use_vwap": [
                False
            ],  # A breakout without volume is a fakeout. Locked to True.
    ...
    "PriceAction": {
        ...
        "params": {
            ...
            "vol_multiplier": [
                ...
                2.0,
                3.0,
            ],  # Distinct, widened tiers of volume shock
            "use_htf": [True],  # Locked
        },
    },
}
